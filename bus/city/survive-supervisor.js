...
async function fetchCandidateUniverseData(client, universe = SURVIVE_CANDIDATE_UNIVERSE) {
...
function formatCandidateUniverseTable(candidateData, fetchedAtIso, marketOpen) {
  // Round 27: a Source column so the table itself, not just internal tagging, tells the reasoning tasks
  // which candidates were human-picked at design time (baseline) vs proposed same-day by a web-search scan
  // (scan) -- these carry different evidentiary weight even though both get the SAME real Alpaca
  // verification below (a review flagged that keeping this distinction purely internal would leave a real
  // evidentiary-tier gap the "verified vs. not-in-table" language didn't capture).
  const rows = candidateData.map((c) => c.ok
    ? `| ${c.symbol} | ${c.source || 'baseline'} | $${c.bid.toFixed(2)} | $${c.ask.toFixed(2)} | ${c.spreadPct === null ? 'n/a' : `$${c.spread.toFixed(4)} (${c.spreadPct.toFixed(3)}%)`} | ${c.fractionable} | ${c.tradable} | ${c.status} |`
    : `| ${c.symbol} | ${c.source || 'baseline'} | -- | -- | -- | -- | -- | DATA FETCH FAILED: ${c.error} |`);
  ...
  const hasScan = candidateData.some((c) => c.source === 'scan');
  const sourceNote = hasScan
    ? '\n\n"scan" candidates were proposed same-day by an automated web-search pass, not pre-vetted by a human -- weigh that as weaker (E0/E1) evidence than "baseline", even though both rows below carry the same real, live Alpaca verification.'
    : '';
  return [
    `Real, verified candidate data (fetched live from Alpaca at ${fetchedAtIso}):`,
    '',
    '| Symbol | Source | Bid | Ask | Spread | Fractionable | Tradable | Status |',
    '|---|---|---|---|---|---|---|---|',
    ...rows,
  ].join('\n') + staleness + sourceNote;
}
...
async function authorNewMission(citizenId, { rehearsal } = {}) {
  ...
  // Round 27: widen beyond the fixed baseline with same-day scan-proposed candidates, IF a fresh cache
  // exists. readScanCache() is a plain, fast, local, no-network file read (never a live dispatch -- see
  // survive-market-scan.js's header for why the real dispatch is decoupled onto its own cadence) wrapped in
  // try/catch so a missing/corrupt/stale cache can never block or alter mission authoring; on any problem
  // this falls back to exactly today's baseline-only behavior.
  let scannedSymbols = [];
  try { const cache = marketScan.readScanCache(); if (cache) scannedSymbols = cache.symbols; } catch (err) { log(`market-scan cache read skipped: ${err.message}`); }
  const universe = [...SURVIVE_CANDIDATE_UNIVERSE, ...scannedSymbols];
  const rawCandidateData = await fetchCandidateUniverseData(client, universe);
  const scannedSet = new Set(scannedSymbols);
  // A scan-proposed candidate gets one more, cheap floor beyond the real Alpaca check: too wide a spread is
  // a real, live signal of thin/illiquid trading a same-day web-search pick could otherwise surface (a
  // review flagged no liquidity floor existed for scanned symbols). Baseline candidates are never filtered
  // here -- they were deliberately, humanly chosen and keep today's exact behavior, including surfacing a
  // failed fetch as a visible "DATA FETCH FAILED" row rather than being silently dropped.
  // Caught live in a real rehearsal run: an after-hours paper quote for a genuine, liquid stock (AAPL) came
  // back with a $0.00 ask, i.e. a NEGATIVE spread -- "<= MAX_SCAN_SPREAD_PCT" alone lets a negative number
  // through, since it's always <= a positive ceiling. A scanned candidate needs a sane, POSITIVE spread, not
  // just a bounded one.
  const candidateData = rawCandidateData
    .map((c) => ({ ...c, source: scannedSet.has(c.symbol) ? 'scan' : 'baseline' }))
    .filter((c) => c.source === 'baseline' || (c.ok && c.bid > 0 && c.ask > c.bid && (c.spreadPct === null || (c.spreadPct >= 0 && c.spreadPct <= marketScan.MAX_SCAN_SPREAD_PCT))));
  let marketOpen = null;
