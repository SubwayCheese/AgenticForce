...
// `to:` field, loads the matching bus/platform/agents/<to>.json config,
...
function main() {
  ...
  if (!agentConfig) {
    console.error(`No agent config found for to: "${task.to}" (looked for bus/platform/agents/${task.to}.json). Known configs: ${listAgentConfigs().join(', ') || '(none)'}`);
    process.exit(1);
  }
