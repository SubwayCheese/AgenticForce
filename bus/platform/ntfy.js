...
const secrets = require('./secrets-broker.js');

// Topic names in code are LOGICAL names and are public in the repo. ntfy.sh topics have no auth -- the name IS the
// secret -- so bus/secrets.local.json (gitignored) can map a logical name to an unguessable real topic with a string key
// NTFY_TOPIC_<LOGICALNAME uppercased>, e.g. NTFY_TOPIC_AGENTVAULTSURVIVE. No mapping = the logical name is used as-is
// (tests and fresh clones behave exactly as before). Added 2026-09-23 before the repo was published.
function resolveTopic(topic, lookup = (k) => secrets.loadSecret(k)) {
  try {
    const mapped = lookup(`NTFY_TOPIC_${String(topic).toUpperCase().replace(/[^A-Z0-9]/g, '')}`);
    if (typeof mapped === 'string' && /^[A-Za-z0-9_-]{16,64}$/.test(mapped)) return mapped;
  } catch (_) { /* fall back to the logical name */ }
  return topic;
}
...
module.exports = { sendNtfy, sanitizeForNtfy, resolveTopic };
