#!/usr/bin/env node
// validate-agent-config.js -- checks a bus/platform/agents/<id>.json config
...
const VALID_OUTPUT_METHODS = new Set(['file', 'stdout', 'stream-json-result']);
const VALID_PROMPT_DELIVERY = new Set(['stdin', 'stdin-stream-json']); // must match agent-engine.js dispatch()
...
function validate(agentId) {
  ...
  if (!config) {
    return { ok: false, problems: [`no config file found for "${agentId}" (looked for bus/platform/agents/${agentId}.json)`] };
  }
