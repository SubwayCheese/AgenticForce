...
const { listAgentConfigs } = require('../platform/agent-engine.js');
const { validate: validateAgentConfig } = require('../platform/validate-agent-config.js');
const { listPendingTaskIds, TASKS_DIR, EXCLUDED_TASK_DIR_NAMES } = require('../platform/run-task.js');
...
function reportAgentConfigs() {
  ...
  if (results.length === 0) {
    console.log('  (none found in bus/platform/agents/)');
    return;
  }
