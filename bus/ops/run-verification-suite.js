...
const runCollab = require('../platform/run-task-collab.js');
const engine = require('../platform/agent-engine.js');
const { validate: validateAgentConfig } = require('../platform/validate-agent-config.js');
const { search: vaultSearch } = require('../platform/vault-search.js');
...
const fleetStatus = require('../fleet/fleet-status.js');
const cryptoSymbols = require('../fleet/crypto-symbols.js');
const alpaca = require('../fleet/alpaca-client.js');
const finnhub = require('../fleet/finnhub-client.js');
const genPilotTasks = require('../fleet/generate-pilot-tasks.js');
...
function testQueueDaemon() {
  ...
  const daemon = spawn('node', [avPaths.script('run-queue-daemon.js')], {
    cwd: VAULT_ROOT,
...
// touches bus/platform/agents/, so there's no cleanup risk of a broken
...
// corresponds to a real bus/platform/agents/*.json config (no fabricated
...
function testAgentGraphFast() {
  ...
  for (const agent of graph.agents) {
    if (!configuredIds.has(agent.id)) problems.push(`agent "${agent.id}" in graph but no bus/platform/agents/${agent.id}.json config`);
...
// for every agent config found in bus/platform/agents/ -- not just Codex
