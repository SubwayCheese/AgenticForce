...
const COMPANY_NAMES = require(avPaths.fleetData('company-names.json')).names;
const CRYPTO_NAMES = require(avPaths.fleetData('crypto-names.json')).names;
...
function strategyRuleText(pilot) {
  const lines = [
    ...
    '   - Time-based: exit after a fixed holding horizon if no invalidation fired first (this backtest uses 5/10/20-trading-day horizons, the same horizons already used in this vault\'s bus/fleet/backtest-screen-score-v2.js, for direct comparability).',
