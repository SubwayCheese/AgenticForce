...
function runScriptOnce(name) {
  try {
    const out = execFileSync('node', [path.join(SCRIPTS_DIR, name)], {
      encoding: 'utf8',
      cwd: VAULT_ROOT,
    });
...
const inboxWatcher = spawn('node', [path.join(SCRIPTS_DIR, 'watch-inbox.js')], {
  cwd: VAULT_ROOT,
...
setTimeout(() => {
  ...
  sendNtfy({
    ...
    message: `2-hour window ended at ${new Date().toISOString()}. Research crew and inbox watcher are no longer running. Re-run bus/scripts/run-continuous.js (or ask Claude to) to start another window.`,
    tags: 'stop_sign',
  })
    .catch((e) => logLine(`ntfy send failed: ${e.message}`))
