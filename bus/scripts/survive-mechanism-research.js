...
// bus/scripts/mechanisms/ still takes one manual step (a human runs the
...
function authorResearchMission() {
  ...
  const payload = [
    ...
    '',
    'draftModuleContent, only if recommendImplementing is true and mechanismId is set: the FULL javascript file content for bus/scripts/mechanisms/<mechanismId>.js, matching the exact interface shape used by bus/scripts/mechanisms/alpaca-live-equity.js (module.exports with id/displayName/requiredSecretNames/isAvailable()).',
  ].join('\n');
...
function checkResearchResults(runTask) {
  ...
  if (parsed.mechanismId && parsed.clearsHardBoundary && parsed.recommendImplementing && parsed.draftModuleContent) {
    const proposedPath = path.join(__dirname, 'mechanisms', `${parsed.mechanismId}.proposed.js`);
