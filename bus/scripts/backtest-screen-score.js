...
const UNIVERSE_PATH = path.join(__dirname, 'fleet-universe.json');
const REPORT_PATH = path.join(__dirname, 'backtest-screen-score-results.md');
...
function markdownReport({ universe, commonDates, signalDates, metrics, verdict, minBars, maxBars, finnhubConfigured }) {
  ...
  return [
    ...
    '',
    `- Universe: the ${universe.length}-symbol equity list in \`bus/scripts/fleet-universe.json\`; no symbols were added or removed.`,
