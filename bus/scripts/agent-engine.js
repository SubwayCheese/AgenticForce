...
// bus/scripts/agents/<id>.json. Everything that doesn't vary (dependency
...
function dispatch(agentConfig, prompt, { mode, cwd, envOverlay }) {
  ...
  if (agentConfig.promptDelivery !== 'stdin') {
    throw new Error(`Agent "${agentConfig.id}" has promptDelivery: "${agentConfig.promptDelivery}" -- only "stdin" is actually implemented in dispatch(). Passing the prompt any other way (e.g. as a positional arg) needs real code here, not just a config value.`);
  }
