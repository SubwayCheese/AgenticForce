...
const RESULTS_PATH = path.join(SCRIPTS_DIR, 'backtest-screen-score-v2-results.md');
...
function maybeRunDailyBacktest() {
  ...
  try {
    execFileSync('node', [path.join(SCRIPTS_DIR, 'backtest-screen-score-v2.js')], { cwd: VAULT_ROOT, encoding: 'utf8', maxBuffer: 20 * 1024 * 1024 });
  } catch (err) {
